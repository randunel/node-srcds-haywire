...
Game.prototype.player_death = function(data) {
    ...
    if(!this.players[data.pId].name) console.log(this.players[data.pId]);
    if(!this.players[data.attId].name) console.log(this.players[data.attId]);
...
Game.attach = function(stats, rcon) {
    ...
    rcon.getRoundStatus(function(err, status) {
        ...
        stats.on('game', function(name, data) {
            ...
            if(!!data.pId && !!!game.players[data.pId]) {
